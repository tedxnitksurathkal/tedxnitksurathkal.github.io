...
let SurfaceComponent = class SurfaceComponent extends BaseComponent {
    constructor(projectData) {
        ...
        this.mode = SurfaceMode.PEN_MODE;
        this.width = 0;
        this.height = 0;
        this.scale = 1;
        this.gap = 20;
        ...
        this.isMoveKeyPressed = false;
        this.isCurrentEventMove = false;
        this.changeThrottle = 0;
        this.onChange = () => { };
    ...
    onResize() {
        ...
        this.rect = {
            width: document.clientWidth || window.innerWidth,
            height: document.clientHeight || window.innerHeight
        };
        const padding = 3;
    ...
    eventInput(type, state, data) {
        ...
        if ((data === null || data === void 0 ? void 0 : data.origin.x) === -1) {
        ...
        if (type === GESTURE.SCALE || this.isCurrentEventMove) {
            ...
            const dataScale = (data === null || data === void 0 ? void 0 : data.scale) || 1;
            ...
            if (this.mode === SurfaceMode.ERASER_MODE) {
                var target = (_a = this.shadowRoot) === null || _a === void 0 ? void 0 : _a.elementFromPoint(data.origin.x + data.drag.x, data.origin.y + data.drag.y);
                if (!target || (target === null || target === void 0 ? void 0 : target.parentNode) !== this.content) {
...
SurfaceComponent = __decorate([
    Component('surface-cmp', './assets/theme/css/surface.style.css')
], SurfaceComponent);
export { SurfaceComponent };
function generateDotId() {
    return 'dot-' + btoa(`${Math.random() * (Math.pow(2, 53))}`);
}
export function generateBaseSVG(data) {
    const gap = 20;
    ...
    <svg id="svg-surface" data-ref="svg" xmlns="http://www.w3.org/2000/svg" class="surface" viewBox="${viewBox.join(' ')}">
      <defs>
        <pattern width="${100 / data.width}%" height="${100 / data.height}%" viewBox="0,0,${gap},${gap}" id="${id}">
          <circle cx="${0.5 * gap}" cy="${0.5 * gap}" r="1" fill="#fff"></circle>
        </pattern>
      </defs>
      <rect data-ref="dots" x="0" y="0" width="${data.width * gap}" height="${data.height * gap}" fill="url('#${id}')"></rect>
      <g data-ref="content" stroke="white" stroke-linecap="round" fill="none" stroke-width="${data.thickness}">${data.content}</g>
    </svg>
  `;
}
